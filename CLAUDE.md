...
**Symbole und Hervorhebung.** Jede Zeile trägt das echte `pixelSprites`-Bild der Einheit in Spielerfarbe (`recapSpriteURL` rendert es einmalig in ein Offscreen-Canvas und cacht die data-URL) — bei Angriffen Angreifer groß, Ziel klein, damit die Zeile als "der da → den da" liest. Antippen wählt die Zeile aus: `window.recapFocus` hebt **alle** ihre Felder auf der Karte hervor (gelesen von `js/render.js` und `js/render3d.js`), die Kamera fährt auf das erste, erneutes Antippen steppt weiter.
