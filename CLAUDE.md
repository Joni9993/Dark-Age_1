...
node maptest/verify_underworld_m12.js     # Dynamit, Moral-Kollaps, Erschließung (Kern/Ausläufer, Pause, Übernahme, Sieg)
```

**Server-Prüfskripte** (ohne DB): `node server/scripts/test-rating.js`, `node server/scripts/test-seating.js`, `node server/scripts/test-turn-guard.js`, `node server/scripts/test-turn-route.js` (letzteres fährt die echte Turn-Route mit gestubbter DB hoch).
...
**Zugübergabe: der Client ist ab dem Absenden nicht mehr am Zug** (Sept 2026, Dannys Meldung „ich konnte plötzlich deine Einheiten bewegen und war du sozusagen"): `doEndTurn` dreht `gameState.cp` auf den nächsten Spieler und lädt den Blob erst danach hoch — die Karte blieb währenddessen stehen **und bedienbar**. Alle Riegel im Client hängen aber an `currentTurnSlot` (`handleCanvasClick`/`handleUnderworldClick` js/input.js, Fraktion/Reliquie/Handel js/ui.js), und das wurde bis dahin **nur beim Laden** gesetzt (js/lobby.js). Das Brett gehörte in diesem Fenster also dem Gegner, mit dessen Sicht und dessen Rechten — auf dem Handy Sekunden lang, bei einem hängenden Request beliebig lange, nach einem Fehlschlag bis zum Neuladen (genau das hat Danny gemacht). Drei Dinge greifen jetzt ineinander:

- **`currentTurnSlot = gameState.cp` beim Absenden** (`submitTurnToServer`, js/input.js) — eine Zuweisung, die alle bestehenden Guards auf einmal scharf stellt, statt an fünf Stellen eine neue Bedingung einzubauen. Der Server bleibt die Wahrheit: beim nächsten Laden kommt der Wert ohnehin frisch aus der Antwort.
- **Der Zwischenscreen kommt sofort, nicht erst mit der Antwort** (`showTurnSubmitPending`, js/lobby.js). Anzeige und Regel decken denselben Fall unabhängig voneinander ab. Schlägt das Hochladen fehl, wird der Zug-beenden-Knopf **nicht** wieder scharf gemacht (so war es vorher) — weiterspielen hieße, den Zug des Nächsten zu spielen. Stattdessen hält `pendingTurnSubmit` den fertigen Blob fest, „Erneut senden" schickt exakt denselben nochmal. Neu serialisieren ginge nicht: lokal ist längst der Nächste dran.
- **Der Server prüft die Weitergabe** (`server/turnorder.js`, eingehängt in `POST /:id/turn`): erlaubt ist genau der nächste Spieler in der Sitzordnung, überspringen darf man nur **tote** Slots. Das fängt den teuersten Folgefall ab — ein zweites „Zug beenden" nach einem fehlgeschlagenen Upload hätte den fremd gespielten Zustand hochgeladen, und `current_slot` stand ja noch auf dem Absender, die alte Prüfung hätte ihn durchgewunken. Bewusst fail-open bei einem unlesbaren Blob (dieselbe Haltung wie die Eliminierungs-Nachbereitung daneben), und bewusst tolerant, wenn `cp` auf einem Toten steht: stirbt der neue aktive Spieler noch im selben Zugende an einem Brand-Tick, ist genau das der korrekte Zustand.

Mitgefixt: **die Diplomatie-Knöpfe waren die einzigen Aktionsknöpfe ohne Readonly-Guard** (js/diplomacy.js). Wer nur zuschaute, konnte „Anfragen" drücken — 5💰/5🪵 wurden lokal abgezogen, der Toast meldete „gesendet", und der nächste Blob des aktiven Spielers überschrieb alles wieder. Die Anfrage kam nie an. Jetzt sperrt `isMyActiveTurn()` sowohl die Knöpfe (mit sprechender Beschriftung, eine offene Anfrage sieht man weiterhin) als auch die Handler selbst.

**Herz der Tiefe / Erschließung** (`js/hex.js` Geometrie, `js/logic.js` Zähler, Korrektur Sept 2026): Die Herzkaverne liegt exakt unter dem zentralen Wachturm und besteht aus zwei bewusst getrennten Teilen. Der **Kern** (`getHeartCoreHexes`, Typ `UW_HERZ`) ist auf **jeder** Kartengröße Zentrum + Ring 1 = 7 Hexes — nur dort zählt die Erschließung, und nur ein Gegner dort hält sie an. Die **Ausläufer** (`getHeartArmHexes`, Typ `UW_HERZWEG`) sind die 6 Sternarme (je ein Achsen-Hex in Ring 2 ab Radius 6, zusätzlich Ring 3 ab Radius 9): begehbar, optisch als Weg zum Herz markiert, ohne jede Spielwirkung. Vorher zählte die ganze Sternfläche mit — auf Radius 12 also 19 statt 7 Hexes, praktisch nicht zu verteidigen. `getHeartCavernHexes` ist weiterhin beides zusammen (Optik/Begehbarkeit).

Der Zähler (`uw.hz = {p, n, pa}`) kennt drei Ausgänge, alle aus `erschliessungStatus`: **+1** (eigene Einheit im Zentrum, kein Gegner im Kern), **Pause** (Gegner im Kern — `pa = 1`, der Stand bleibt erhalten, aber kein Sieg) und **Reset auf 0** (Mitte leer *ohne* gegnerischen Druck — sonst ließe sich Fortschritt horten). Ein Gegner, der bis ins Zentrum vorstößt, übernimmt und startet bei 1. Dieselbe Dreiteilung gilt für die finale Sieg-Prüfung am Rundenende in `doEndTurn`. `killPlayer` löscht den Zähler des Toten mit — sonst blockiert ein verwaister Zähler jede weitere Erschließung, weil `advanceErschliessung` fremde Zähler nur über die Übernahme anfasst.
