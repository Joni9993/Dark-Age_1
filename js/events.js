...
const gameEvents = [
    ...
    { icon: '🏰', name: 'Befestigung', desc: 'Die Dörfer werden verstärkt! Alle Hauptgebäude heilen +5 HP.', effect: (gs) => { gs.p.forEach(p => { if (!p.dead) p.sh = Math.min(getVillageMaxHp(p), p.sh + 5); }); } }
];
