...
function loadGameCode() {
    ...
    const fn = new Function(src + `
        return {
            ...
            getUnitMaxHp, getUnitCost, getUnitMove
        };
    `);
    return fn();
}
...
console.log('=== (a) calculateMovesUW bewegt nur über offene Hexes ===');
{
    ...
    const unit = { i: 1, p: 0, t: 16, x: startHex.x, y: startHex.y, h: 8, a: 0 };
...
console.log('\n=== (b) Graben öffnet genau das Ziel-Hex und erzeugt Lärm ===');
{
    ...
    const unit = { i: 1, p: 0, t: 16, x: start.x, y: start.y, h: 8, a: 0 };
    state.uw.u.push(unit);
...
console.log('\n=== (c) Ader-Abbau 4x -> Hex offen, cr korrekt, Abliefern bucht auf p[].k ===');
{
    ...
    if (aderHex) {
        const unit = { i: 1, p: 0, t: 16, x: aderHex.x, y: aderHex.y, h: 8, a: 0 };
    ...
    if (aderHex2) {
        const unit2 = { i: 2, p: 1, t: 16, x: aderHex2.x, y: aderHex2.y, h: 8, a: 0 };
        const sequence = [];
...
console.log('\n=== (d) Serialisierungs-Roundtrip: uw.u/uw.d/p[].k/p[].ue verlustfrei, Defaults entfernt ===');
{
    ...
    state.uw.u.push({ i: 1, p: 0, t: 16, x: 3, y: 3, h: 8, a: 0, cr: 2, vet: 1 });
...
console.log('\n=== (e) Stollenkopf-Ableitung: Tunnel bauen -> Hex darunter offen + kaufbar, Tunnel zerstört -> zu ===');
{
    ...
    // Kaufbarkeit: buyUWUnitAt darf am Stollenkopf funktionieren
    const beforeGold = state.p[1].g;
    const unit = M.buyUWUnitAt(state, 1, targetHex.x, targetHex.y, 16);
    assert(unit.t === 16 && unit.x === targetHex.x && unit.y === targetHex.y, 'Tunnelgräber wird korrekt am Stollenkopf angelegt');
    assert(state.p[1].g === beforeGold - M.getUnitCost(state.p[1], 16), 'Gold wurde um die Tunnelgräber-Kosten reduziert');
